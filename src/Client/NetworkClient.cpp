...
Jetpack::Client::NetworkClient::NetworkClient(const int serverPort, std::string serverAddress, const bool debugMode)
    : m_serverPort(serverPort), m_serverAddress(std::move(serverAddress)), m_debugMode(debugMode) {}
...
bool Jetpack::Client::NetworkClient::connectToServer() {
  ...
  if (::connect(m_serverSocket, reinterpret_cast<struct sockaddr *>(&serverAddr),
                sizeof(serverAddr)) < 0) {
    ::close(m_serverSocket);
    throw Jetpack::Shared::Exceptions::SocketException(
...
void Jetpack::Client::NetworkClient::networkLoop() {
    ...
    while (m_running) {
        ...
        if (bytesRead > 0) {
            ...
            processPacket(recvBuffer, bytesRead);
        } else if (bytesRead < 0) {
...
void Jetpack::Client::NetworkClient::processPacket(const uint8_t *data, size_t length) {
    ...
    switch (auto packetType = static_cast<Shared::Protocol::PacketType>(data[0])) {
        ...
        default:
            if (m_debugMode) {
                std::cout << "Unknown packet type: " << static_cast<int>(packetType) << std::endl;
            }
            break;
    }
}
...
void Jetpack::Client::NetworkClient::handleMapData(const uint8_t *data, const size_t length) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            m_map.tiles[y][x] = static_cast<Shared::Protocol::TileType>(data[5 + y * width + x]);
        }
    }
...
void Jetpack::Client::NetworkClient::handleGameStart(const uint8_t *data, const size_t length) {
    ...
    if (m_debugMode) {
        std::cout << "Game starting with " << playerCount << " players" << std::endl;
    }
...
void Jetpack::Client::NetworkClient::handleGameStateUpdate(const uint8_t* data, const size_t length) {
    ...
    for (int i = 0; i < playerCount; i++) {
        ...
        const auto state = static_cast<Shared::Protocol::PlayerState>(data[offset + 1]);
...
void Jetpack::Client::NetworkClient::handleCoinCollected(const uint8_t* data, const size_t length) const {
    ...
    if (m_debugMode) {
        std::cout << "Player " << playerId << " collected coin at (" << x << "," << y << "), new score: " << newScore << std::endl;
    }
